...
def feature_extraction(df_,key, n_chars, n_features, lists=False):
    ...
    for col in df_: #change to df_[df_.columns[0:3]]
        if lists:
...
def load_sherlock_data():
    os.chdir(r'C:/Users/jgtha/Dropbox/sherlock-project-master')
    data = pd.read_parquet("data/data/raw/test_values.parquet").reset_index(drop=True)
    labels = pd.read_parquet("data/data/raw/test_labels.parquet").reset_index(drop=True) 
    os.chdir(r'C:/Users/jgtha/Dropbox/Default Python Directory/')
...
def load_rammandan_city_data(range_start, range_end):
    data=[]
    labels=[]
    for i in range(range_start,range_end):
        file_path='data/rammandan_city/s{}.txt'.format(i)
        f=open(file_path)
        source=f.readlines()
...
if __name__=='__main__':
    keys=['uniprot', 'lld', 'old', 'nbdc', 'dbpedia', 'wikidata', 'rammandan', 'sherlock']
    key='uniprot'
    n_chars=95 #includes 98% of whole cells for uniprot
    n_features=6
    tic = time.perf_counter() 
    
    X_train, X_test, y_train, y_test, dict_labels=load(key)
   
    #df=pd.read_excel(r'data/{}_dt_properties.xlsx'.format(key),engine='openpyxl',parse_dates=True)
    #_, _, dt=get_datatype_stats(df, nl_tresh=0.5)
    ...
    idx=list(np.where(labels==16)[0][0:10])
    df=df_train[df_train.columns[idx]].iloc[0,:20]
    df1=df.tolist()
    df2=pd.DataFrame(df1).transpose()
    '''  
    ...
    #rammandan feature extraction
    '''
    key='rammandan'
    df_train, ser_labels=load_rammandan_city_data(1, 6)
    df_test, ser_labels=load_rammandan_city_data(6, 11)
    train_doc2vec_model(df_train, key, n_chars, lists=True)
    df_train=feature_extraction(df_train, key, n_chars, n_features, lists=True)
    df_test=feature_extraction(df_test, key, n_chars, n_features, lists=True)
    X_train, X_test=preprocess(df_train, df_test)
