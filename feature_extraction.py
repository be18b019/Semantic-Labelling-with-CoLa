...
from column_feature_extraction import get_glove_dict
...
def save_word(X_train, X_test, ser_labels, key):
    print('saving...')   
    save_npz(r'{}word_train_data.npz'.format(key), X_train)
    save_npz(r'{}word_test_data.npz'.format(key), X_test)
    file_name=r'{}word_properties.xlsx'.format(key)
    with pd.ExcelWriter(file_name) as writer:
...
def save_(X_train, X_test, ser_labels, key):
    print('saving...')
    save_npz(r'data/extracted_features/{}_train_data.npz'.format(key), X_train)
    save_npz(r'data/extracted_features/{}_test_data.npz'.format(key), X_test)
    file_name=r'data/extracted_features/{}_properties.xlsx'.format(key)

    with pd.ExcelWriter(file_name) as writer:
        ser_labels.to_excel(writer, index=False)
    print('saved!')

def load_word(key, samples):
    print('loading data...')
    
    X_train = load_npz(r'{}word_train_data.npz'.format(key))
    X_test = load_npz(r'{}word_test_data.npz'.format(key))
    dict_labels=pd.read_excel(r'{}word_properties.xlsx'.format(key),engine='openpyxl').to_dict()[0]
...
def load_(key, samples):
    print('loading data...')
    X_train = load_npz(r'data/extracted_features/{}_train_data.npz'.format(key))
    X_test = load_npz(r'data/extracted_features/{}_test_data.npz'.format(key))
    dict_labels=pd.read_excel(r'data/extracted_features/{}_properties.xlsx'.format(key),engine='openpyxl').to_dict()[0]
...
if __name__=='__main__': 
    ...
    '''
    df=pd.read_excel(r'data/{}_dt_properties.xlsx'.format('nbdc'),engine='openpyxl',parse_dates=True)
    dict_labels = dict(enumerate(column.strip() for column in df.columns))
    ser_labels=pd.Series(dict_labels)
    list_df=[pd.read_excel(r'{}_{}.xlsx'.format('nbdc', i),engine='openpyxl') for i in range(26)]
    df_feat=pd.concat(list_df, ignore_index=True) 
    for i in range(df_feat.shape[0]):
        print(i)
        df_feat.iloc[i,0]=[np.array(literal_eval(df_feat.iloc[i,0]))]
    ...
    X_train, X_test,=preprocess_features(df_feat, n_features, n_chars)
    save_word(X_train, X_test, ser_labels, key)
    
    X_train, X_test, y_train, y_test, dict_labels=load(key, samples)
    X_train_word, X_test_word, y_train_word, y_test_word, dict_labels_word=load_word(key, samples)
    ser_labels=pd.Series(dict_labels)
    
    X_train=hstack([X_train, X_train_word], format='csr')
    X_test=hstack([X_test, X_test_word], format='csr')
