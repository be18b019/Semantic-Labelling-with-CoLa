...
from feature_extraction import load 
from column_feature_extraction import load as col_load
from dataset_stats import get_datatype_stats
...
def classify(classifier, X_train, y_train, X_test, y_test, dict_labels, key):
    ...
    if classifier=='RF':
        clf= RandomForestClassifier(random_state=42, max_depth=20, n_estimators=50) #,n_estimators=50 for cell approach #without max_depth memory overload
...
def main(samples, keys,classifiers, columns=False):
    ...
    make_file(arr_scores_params, keys, classifiers, 'measurement_different_models_databases.xlsx')

def mixtest(samples, classifiers,columns=False):
    ...
    make_file(arr_scores_params, [0], classifiers, 'measurement_mixtest_uniprot_lld.xlsx')
    return arr_scores_params

def smash_test(samples, keys,classifiers,columns=False):
    arr_scores_params=np.empty(shape=(len(classifiers),len(keys)), dtype=object)
    for key in range(len(keys)):
        for classifier in range(len(classifiers)):
            gc.collect()
            ...
            arr_scores_params[classifier, key]=ser_scores_params
    make_file(arr_scores_params, keys, classifiers, 'measurement_smashtest.xlsx')
...
if __name__=='__main__':
    ...
    #X_train_alpha, y_train_alpha, X_test_alpha, y_test_alpha, dict_labels=alpha_num_separation(key, samples, alpha=True, columns=columns)
    #X_train_num, y_train_num, X_test_num, y_test_num, dict_labels=alpha_num_separation(key, samples, alpha=False, columns=columns)
    
    #ser_scores_params, df_cm, report, clf=classify(classifier, X_train_alpha, y_train_alpha, X_test_alpha, y_test_alpha, dict_labels, key)
    #ser_scores_params, df_cm, report, clf=classify(classifier, X_train_num, y_train_num, X_test_num, y_test_num, dict_labels, key)
    ...
    #done alpha_num_test(samples, keys,classifiers,columns=True, alpha=True, filename='col_alpha_measurement.xlsx')
    #done alpha_num_test(samples, keys,classifiers,columns=True, alpha=False, filename='col_num_measurement.xlsx')
    ...
    mixtest(samples, classifiers,columns=columns)
    smash_test(samples, train_keys,classifiers,columns=columns)
    
    
    #sample classification for cell approach
    '''
    X_train, X_test, y_train, y_test, dict_labels=load(key, samples)
    ser_scores_params, df_cm, report, clf=classify(classifier, X_train, y_train, X_test, y_test, dict_labels, key)
    '''
    
    #sample classification for column approach
    '''
    X_train, X_test, y_train, y_test, dict_labels=col_load(key) 
    ser_scores_params, df_cm, report, clf=classify(classifier, X_train, y_train, X_test, y_test, dict_labels, key)
    '''
